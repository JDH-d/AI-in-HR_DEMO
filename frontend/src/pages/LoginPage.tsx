...
export function LoginPage() {
  ...
  const [selected, setSelected] = useState<Role>(() => roleForDestination(requestedNext) ?? "employee"); const [password, setPassword] = useState("demo-password");
  const [error, setError] = useState(""); const [busy, setBusy] = useState(false);
  if (user) return <Navigate to={destinationForRole(user.role, requestedNext)} replace />;
  const submit = async () => { setBusy(true); setError(""); try { await login(selected, password); navigate(destinationForRole(selected, requestedNext)); } catch (e) { setError(e instanceof Error ? e.message : "Login failed"); } finally { setBusy(false); } };
...
export function destinationForRole(role: Role, requestedNext: string | null): string {
  const defaultPath = roles.find(item => item.id === role)?.path ?? "/";
  if (
    requestedNext === defaultPath
    || requestedNext?.startsWith(`${defaultPath}?`)
    || requestedNext?.startsWith(`${defaultPath}#`)
  ) {
...
function roleForDestination(requestedNext: string | null): Role | null {
  return roles.find(item =>
    requestedNext === item.path
    || requestedNext?.startsWith(`${item.path}?`)
    || requestedNext?.startsWith(`${item.path}#`)
  )?.id ?? null;
}
