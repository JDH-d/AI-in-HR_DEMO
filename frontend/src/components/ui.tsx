...
export function Button({
  ...
  return (
    <button
      className={clsx(
        "focus-ring inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition active:scale-[.98] disabled:pointer-events-none disabled:opacity-45",
...
export function Card({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={clsx("rounded-2xl border border-line bg-panel p-5", className)}>{children}</div>
  );
}
export function Badge({
  ...
  return (
    <span
      className={clsx(
        "inline-flex rounded-full border px-2.5 py-1 text-[11px] font-bold uppercase tracking-[.12em]",
        tone === "neutral" && "border-line bg-raised text-muted",
        tone === "success" && "border-lime/25 bg-lime-soft text-lime",
        tone === "warning" && "border-coral/25 bg-coral/10 text-coral",
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          className={clsx(
            ...
            placement === "center" &&
              "left-1/2 top-1/2 max-h-[calc(100vh-2rem)] max-w-[780px] -translate-x-1/2 -translate-y-1/2 rounded-3xl",
          )}
        >
...
export const fieldClass =
  "focus-ring w-full rounded-xl border border-line bg-ink px-3.5 py-3 text-sm text-cream placeholder:text-muted/60";
