...
export function RequestDrawer({ open, onOpenChange, initial }: { open: boolean; onOpenChange: (v:boolean)=>void; initial?: WorkflowRequest | null }) {
  ...
  useEffect(()=>{
    if(initial){setType(initial.type);setStart(initial.start_date??"");setEnd(initial.end_date??"");setComment(initial.comment);setApprover(initial.approver||"manager.demo");}
    else {setType("pto");setStart("");setEnd("");setComment("");setApprover("manager.demo");setError("");}
  },[initial, open]);
  ...
  return <Drawer open={open} onOpenChange={onOpenChange} title={initial?"Review request":"Create request"} description="Nothing is sent until you confirm. Every later decision is added to the timeline.">
    <div className="mb-6 flex items-center justify-between"><Badge tone={statusTone(initial?.status??"draft")}>{initial?.status?formatStatus(initial.status):"new draft"}</Badge><span className="text-xs text-muted">Required fields marked *</span></div>
    ...
    <Card className="grid grid-cols-2 gap-4 bg-ink/50 p-4 text-xs"><Summary icon={<CalendarDays size={16}/>} label="Period" value={start&&end?`${start} → ${end}`:"Not set"}/><Summary icon={<Clock3 size={16}/>} label="Duration" value={duration?`${duration} day${duration===1?"":"s"}`:"—"}/><Summary icon={<UserRound size={16}/>} label="Applicant" value="You"/><Summary icon={<Check size={16}/>} label="Next status" value="In review"/></Card>
