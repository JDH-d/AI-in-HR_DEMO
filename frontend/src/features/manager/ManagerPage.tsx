...
export function ManagerPage() {
  ...
  const rows = useMemo(
    () => data.filter(request =>
      ...
      && `${request.type_label} ${request.applicant} ${request.comment}`.toLowerCase().includes(search.toLowerCase()),
    ),
    [data, status, search],
  );
  const sidebar = <>
      ...
      <Card className="mt-8 overflow-hidden p-0">
        <div className="flex flex-col gap-3 border-b border-line p-4 sm:flex-row">
          <label className="relative flex-1">
            ...
            <input className={`${fieldClass} pl-10`} value={search} onChange={event => setSearch(event.target.value)} placeholder="Search requests" />
          </label>
...
function Metric({ label, value, icon }: { label: string; value: number; icon: React.ReactNode }) {
  return <Card className="min-w-24 p-3">
...
function DecisionDialog({ id, onClose, onChanged }: { id: string | null; onClose: () => void; onChanged: () => void }) {
  const { token } = useAuth();
  ...
  const action = useMutation({
    ...
    onError: requestError => setError(requestError instanceof Error ? requestError.message : "Decision failed"),
  });
  ...
  >
    ...
    {request && detail.data && <div className="space-y-6">
      ...
      {request.status === "in_review" && <section className="rounded-2xl border border-line bg-ink/35 p-5">
        ...
        <p className="mt-2 text-[11px] text-muted">The employee will see this note with the final decision.</p>
        {error && <p className="mt-3 text-sm text-danger">{error}</p>}
        <div className="mt-4 grid grid-cols-2 gap-3">
          <Button onClick={() => action.mutate("approve")} disabled={action.isPending}>Approve</Button>
          <Button tone="danger" onClick={() => action.mutate("decline")} disabled={action.isPending || !comment.trim()}>Decline</Button>
        </div>
      </section>}
